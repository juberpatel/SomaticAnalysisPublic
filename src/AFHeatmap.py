...
def createPairtreeInputs(patientName, df, tr):
    ...
    for mutation, group in mutationGroups:
        ...
        for i in range(len(samples)):
            total_reads[i] = trDict[samples[i]][mutation]
...
def main():
    ...
    patient = "MSK-AB-0002"
    d = "/Users/patelj1/current/Autopsy/analysis/yj-filtered-new/jlab_and_cmo.filtered/" + patient + "/filtered/heatmap"
    os.chdir(d)
    mutationsFile = patient + "-ABSOLUTE-oncokb.tsv"
    totalReadsFile = d + "/../../../" + patient + ".genotyped_mutations.jlab_and_cmo.filtered.maf"
    patientName = patient + "-absolute"
    badSamplesFile = patient + "-bad-samples.txt"
    copyNumberFile = patient + "-copy-numbers.txt"
    absoluteSummaryFile = d + "/Output/absolute_results/Output/reviewed/absolute.test.ABSOLUTE.table.txt"
    ...
    for group, labels in groups.items():
        #if len(labels)<5 and ";Cancer" not in "".join(labels):
        # values used: 1: 30, 2,3,6: 10
        if len(labels)<1: #and ("HOTSPOT" not in "".join(labels)) and ("Onco" not in "".join(labels)):
            print(group, "\t", labels)
    ...
    r = sb.clustermap(absolute, method="complete", metric="canberra", col_cluster=False, 
                      cmap=c, vmin=0.01, vmax=1.0, cbar_kws={'label': 'CCF'}, figsize=(25, 10),
